...
alSaveTimer = alSaveTimer or Timer(5, function(timer)
		if alDirty then
			local xok, xerr = serialize(alData, "armleg.new.dat" .. (datx or ''), filesync)
			if xok then
				os.remove("armleg.old.dat" .. (datx or ''))
				assert(os.rename("armleg.dat" .. (datx or ''), "armleg.old.dat" .. (datx or '')))
				assert(os.rename("armleg.new.dat" .. (datx or ''), "armleg.dat" .. (datx or '')))
				-- os.remove("armleg.old.dat" .. (datx or ''))
				alDirty = false
			else
